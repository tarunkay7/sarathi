...
function buildTools(availableDates, slotTimes) {
  ...
  return [
    ...
    },
    {
      ...
      description: 'Call this once the citizen has confirmed their documents. If any document must be attached as a file, this will fail until they have actually attached it on screen — the error names what is still missing, so relay that and wait rather than retrying immediately.',
      parameters: { type: 'object', properties: {}, required: [] },
    },
    {
      type: 'function',
    ...
    },
    {
      type: 'function',
      name: 'end_call',
      description: 'Call this to hang up — once the citizen says goodbye/thanks/"that\'s all" (normally after finish), or if they want to stop partway through. Always say a brief warm goodbye out loud in the SAME turn first; never call this silently.',
      parameters: { type: 'object', properties: {}, required: [] },
    },
  ];
}
...
router.post('/session', asyncHandler(async (req, res) => {
  ...
  const { citizenName, dob, vehicleClasses, rto, formNumber, feeRupees, requiresSlot, form1a, checklist, uploads, earliestSlotDate, availableDates, slotTimes,
    serviceTitle, serviceAction, prerequisiteNote, needsLearnerLicence } = req.body || {};
  ...
  const instructions = [
    `You are Setu, a warm, genuinely helpful voice assistant who actually DRIVES ${firstName} through ${serviceAction || 'renewing their Indian driving licence'} — you are not a chatbot answering questions, you are the one operating the process for them. All data is mock/demo; never claim to contact a real government system.`,
    `Citizen: ${citizenName || 'the citizen'}, date of birth ${dob || 'unknown'}, licence class ${vehicleClasses || 'unknown'}, RTO ${rto || 'unknown'}.`,
    `Service: ${serviceTitle || 'Driving Licence Renewal'}. Form: ${formNumber || 'Form 9'}. Fee: ₹${feeRupees != null ? feeRupees : '400'}.`,
    prerequisiteNote
      ? `Before anything else, state this requirement plainly and warmly, once: "${prerequisiteNote}" Ask ${firstName} to confirm they meet it before you go on.`
      : '',
    needsLearnerLicence
      ? `IMPORTANT — this application is issued against a learner's licence, and you need its number. You cannot hear it reliably and must not guess it: there is a box on their screen headed "Learner's licence number". Right after begin_intake, ask ${firstName} to TYPE the number from their learner's licence into that box and press Submit. Then WAIT. Do not call start_application until you receive a bracketed [system] note telling you the number arrived — if you call it early it will fail and tell you to wait. When the note arrives, read the number back to them once to confirm, then continue.`
      : '',
    docLines.length ? `Documents needed: ${docLines.join('; ')}.` : '',
    uploads && uploads.length
      ? `IMPORTANT — these must be attached as actual files and you cannot do it for them: ${uploads.join(', ')}. When you reach the documents step, tell ${firstName} plainly that there is an upload box on their screen for each one and ask them to attach it. Then WAIT — do not call confirm_documents until you are told the upload landed. You will receive a bracketed [system] note the moment each file arrives; acknowledge it warmly in one short line. If you call confirm_documents too early it will fail and tell you what is still missing, so just relay that and keep waiting.`
      : 'Nothing needs to be uploaded as a file for this service — the documents step is a spoken confirmation only.',
    ...
    `Good flow, in order: (0) greet ${firstName} warmly by first name in one short line, then ask a plain yes/no question: "are you ready to start your ${serviceTitle ? serviceTitle.toLowerCase() : 'renewal'} application?" — and wait for their answer. Do not read out any details or call any tool yet. (0.5) The moment they say yes (or an equivalent clear affirmative), immediately call begin_intake — this reveals their application card on screen — before saying anything else. (1) Once begin_intake has been called, read back their key details in one short line — name, date of birth, licence class, and RTO — and explicitly ask them to confirm those are correct. Do NOT call start_application yet. If they say something is wrong, apologise and note it, but continue (this is a demo, nothing can actually be corrected here).${needsLearnerLicence ? ` (1.5) After they confirm their details, ask them to TYPE their learner's licence number into the box on screen and press Submit, then WAIT for the bracketed [system] note confirming it arrived. Read the number back once, then call start_application.` : ' Only once they confirm (or correct and then confirm) do you call start_application.'} (2) name the required documents (mentioning the DigiLocker-fetched photo and asking them to confirm that's fine, plus Form 1A if applicable) and ask them to confirm they're ready, then call confirm_documents — but if anything has to be attached as a file, ask them to attach it first and wait for the [system] note confirming it arrived before calling confirm_documents, (3) if a slot is required: as SOON as the citizen names or agrees to any date and time (even a first guess), immediately call select_slot with it so their screen updates live with that proposed card — do this before asking for final confirmation, not after. If they then change their mind to a different date/time, call select_slot again with the new one to update the card live. Only after they explicitly confirm the proposed slot (a plain "yes"/"that works"/etc.) do you call confirm_slot to lock it in and move on. (4) ask which payment method they'd like and call make_payment as soon as they answer, (5) once payment succeeds, congratulate them warmly, mention that a confirmation has been sent to their email and phone number, call finish, and read back the reference number clearly — say all of this before the citizen is taken to their application status page.`,
    ...
    'Ending the call: after finish, once the citizen says thanks/bye/"that\'s all" or otherwise signals they are done, say a short warm goodbye by name and call end_call. If they want to stop before finishing, say a brief goodbye acknowledging that and call end_call anyway — never leave a call trailing off without saying goodbye and calling end_call. Speak briskly and get to the point — the citizen is watching the screen update as you talk, so do not slow down to narrate what they can already see. Move straight from finishing one step into the next without a beat of dead air.',
  ].filter(Boolean).join(' ');
      ...
      body: JSON.stringify({
        ...
        session: {
          ...
          audio: {
            ...
            // Tunable without a redeploy: pace is the thing most worth
            // adjusting by ear, and 0.88 dragged badly on a timed demo.
            output: { voice: 'alloy', speed: Number(process.env.OPENAI_VOICE_SPEED) || 1.1 },
          },
